...
const Post = ({ post, isPostDetails, isInsideSheared = false }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm mb-6 max-w-md mx-auto md:max-w-3xl overflow-hidden font-sans relative">
      ...
      {/* 3. Post Image */}
      {isShare ? (
        <Post isPostDetails={false} post={sharedPost} isInsideSheared={true} />
      ) : (
        image && (
          <Link
            to={`/postDetails/${_id}`}
        ...
        <>
          ...
          <div className="flex justify-between items-center px-2 py-1 mx-2 mb-1">
            <button
              ...
              disabled={isPending}
              className={`flex-1 cursor-pointer flex items-center justify-center gap-2 py-2 rounded-md transition-colors hover:bg-gray-100 font-medium ${
            ...
            <button
              ...
              className="flex-1 cursor-pointer flex items-center justify-center gap-2 py-2 rounded-md transition-colors hover:bg-gray-100 text-gray-600 font-medium"
            >
              <FaShare size={18} />
              <span>Share</span>
            </button>
